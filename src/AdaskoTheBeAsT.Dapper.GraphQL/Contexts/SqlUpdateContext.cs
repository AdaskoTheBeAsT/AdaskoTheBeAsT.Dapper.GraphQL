...
public class SqlUpdateContext
{
        ...
        dynamic? parameters = null)
    {
        ...
        _updateParameterNames = new HashSet<string>(
            GetParameterNameSource(flatProperties, parameters, Parameters),
    ...
    private static IEnumerable<string> GetParameterNameSource(
        IEnumerable<KeyValuePair<string, object?>>? flatProperties,
        object? parameters,
        DynamicParameters dynamicParameters)
    {
        if (flatProperties != null)
        {
            return flatProperties.Select(kvp => kvp.Key);
        }

        if (parameters is IEnumerable<KeyValuePair<string, object>> kvpEnum)
        {
            return kvpEnum.Select(kvp => kvp.Key);
        }
    ...
    private string BuildSql(string parameterPrefix)
    {
        ...
        sb.AppendJoin(", ", _updateParameterNames.Select(name => $"{name} = {parameterPrefix}{name}"));
#else
